...
def generate_and_save_images(model, model_name, sampler, device, out_path, sample_fn):
    model.eval()
    images = []
    with torch.no_grad():
    ...
    plt.savefig(os.path.join(out_path, 'generated_images_for_debug_bottom', f'model_{model_name}.png'))
    plt.close()
    model.train()
...
def main():
    ...
    parser.add_argument('--model_path', type=str, default='results/checkpoint_epoch_bottom_10000.pth')
    # Training
    ...
    generate_and_save_images(model, model_name, sampler, device, args.save_dir, sample_fn)
